...
function sostavChisla(massivChisel, chislo) {
  // вспомогательная функция, я бы написал её снаружи, но есть требование писать всё здесь
  function sumArray(array) {
    return array.reduce((partialSum, item) => partialSum + item, 0)
  }

  // проверка граничных условий, возможно сумма всех чисел в массиве меньше нужной, или равна ей
  // в таком случае нет смысла искать комбинации
  if (sumArray(massivChisel) < chislo) {
    return [];
  } else if (sumArray(massivChisel) === chislo) {
    return [massivChisel];
  }

  // переменная для хранения уникальных комбинаций.
  let uniqueSets = [[]];

  // Чтобы получить уникальные комбинации чисел из множества, итерируем множество, и добавляем каждый
  // элемент в каждую из уже имеющихся комбинаций, записывая получившуюся комбинацию.

  for (const el of massivChisel) {
    const lastIndex = uniqueSets.length-1;
    for (let i = 0; i <= lastIndex; i++) {
      uniqueSets.push( [...uniqueSets[i], el] );
    }
  }

  // переменная для хранения подходящих комбинаций
  let result = [];

  // проверяем, какие из уникальных комбинаций дают нужную сумму и сохраняем их в ответ
  uniqueSets.map(el => {
    if (sumArray(el) === chislo) {
      result.push(el);
    }
  });
